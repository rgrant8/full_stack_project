...
$(document).ready(function() {
  ...
  var bottom = $(this).height()-5-$(window).height();
    //if (dHeight >= $(this).scrollTop()){
    if (dHeight >= $(this).scrollTop() && $(this).scrollTop() < bottom) { 
      $('header').css('background', 'rgba(7, 105, 167,' + (0.3 + ($(this).scrollTop() / 550)) + ')'); 
        $("#foot-title").fadeOut();}
    else {
       $('header').css('background', 'rgba(7,105,167, 0)'); 
        $("#foot-title").fadeIn();}

    if (dHeight >= $(this).scrollTop() && $(this).scrollTop() < bottom - 300) {
      $("#intro-btn-wrapper").fadeOut();}

    else{
      $("#intro-btn-wrapper").fadeIn();}
  });

  $(window).load(function() {
  $("body").removeClass("preload");});

  setTimeout(function () { $("#filler-three").addClass("is-visible"); },1000)
});
